...
import com.vid.allvideodownloader.model.Edge;
import com.vid.allvideodownloader.model.EdgeSidecarToChildren;
import com.vid.allvideodownloader.model.FBStoryModel.NodeModel;
import com.vid.allvideodownloader.model.ResponseModel;
import com.vid.allvideodownloader.model.story.FullDetailModel;
import com.vid.allvideodownloader.model.story.StoryModel;
...
import static com.vid.allvideodownloader.util.Utils.RootDirectoryInsta;
...
public class InstagramActivity extends AppCompatActivity implements UserListInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DatabaseReference adsBoolean = firebaseDatabase.getReference("instaAds");

        adsBoolean.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                Log.i("Taginsta",""+snapshot.child("instaInter").getValue());
                Log.i("Taginsta",""+snapshot.child("instaNative").getValue());
                Log.i("Taginsta",""+snapshot.child("instaBanner").getValue());

                instaInter = (boolean) snapshot.child("instaInter").getValue();
                instaNative = (boolean) snapshot.child("instaNative").getValue();
                instaBanner = (boolean) snapshot.child("instaBanner").getValue();
    ...
    private void callDownload(String Url) {
        ...
        StringRequest request=new StringRequest(Request.Method.GET, UrlWithoutQP, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                try {
                    ...
                    for (int i=0;i<1;i++){
                        ...
                        String id=shortcode_media.getString("id");
                        boolean is_video= Boolean.parseBoolean(shortcode_media.getString("is_video"));
                        if (is_video){
                            String video_url= shortcode_media.getString("video_url");
                            startDownload(video_url, RootDirectoryInsta, activity, getImageFilenameFromURL(video_url));
                            Log.i("Tag3", String.valueOf(video_url));
                        }else{
                            String display_url = shortcode_media.getString("display_url");
                            startDownload(display_url, RootDirectoryInsta, activity, getImageFilenameFromURL(display_url));
                            Log.i("Tag3", String.valueOf(display_url));
                        }
                        Log.i("Tag1", String.valueOf(graphql));
                        Log.i("Tag2", String.valueOf(shortcode_media));
                        Log.i("Tag4", String.valueOf(is_video));
